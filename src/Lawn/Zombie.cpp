...
constinit const ZombieDefinition gZombieDefs[NUM_ZOMBIE_TYPES] = {
	...
	{ .mZombieType = ZOMBIE_TRAFFIC_CONE, .mReanimationType = REANIM_ZOMBIE, .mZombieValue = 1, .mStartingLevel = 3, .mFirstAllowedWave = 1, .mPickWeight = 3500, .mZombieName = "CONEHEAD_ZOMBIE" },
	{ .mZombieType = ZOMBIE_POLEVAULTER, .mReanimationType = REANIM_POLEVAULTER, .mZombieValue = 1, .mStartingLevel = 6, .mFirstAllowedWave = 3, .mPickWeight = 2500, .mZombieName = "POLE_VAULTING_ZOMBIE" },
	{ .mZombieType = ZOMBIE_PAIL, .mReanimationType = REANIM_ZOMBIE, .mZombieValue = 2, .mStartingLevel = 8, .mFirstAllowedWave = 5, .mPickWeight = 2000, .mZombieName = "BUCKETHEAD_ZOMBIE" },
	{ .mZombieType = ZOMBIE_NEWSPAPER, .mReanimationType = REANIM_ZOMBIE_NEWSPAPER, .mZombieValue = 1, .mStartingLevel = 11, .mFirstAllowedWave = 1, .mPickWeight = 2000, .mZombieName = "NEWSPAPER_ZOMBIE" },
	{ .mZombieType = ZOMBIE_DOOR, .mReanimationType = REANIM_ZOMBIE, .mZombieValue = 2, .mStartingLevel = 13, .mFirstAllowedWave = 4, .mPickWeight = 2500, .mZombieName = "SCREEN_DOOR_ZOMBIE" },
	
	// يجب تعديل الباقي بعدين
...
void Zombie::UpdatePlaying()
{
	...
	if (mIsPulledByPotato)
    {
        float aPullSpeed = 12.0f; // يمكنك تعديل سرعة السحب من هنا

    // تحريك الزومبي تدريجياً أفقياً
        if (mPosX < mPullTargetX)
            mPosX = std::min(mPosX + aPullSpeed, mPullTargetX);
        else if (mPosX > mPullTargetX)
            mPosX = std::max(mPosX - aPullSpeed, mPullTargetX);

    // تحريك الزومبي تدريجياً رأسياً
        if (mPosY < mPullTargetY)
            mPosY = std::min(mPosY + aPullSpeed, mPullTargetY);
        else if (mPosY > mPullTargetY)
            mPosY = std::max(mPosY - aPullSpeed, mPullTargetY);
	...
	if (!IsImmobilizied())
	{
		...
		CheckForHighGround();
		CheckForBoardEdge();
		int aCurrentTileX = mBoard->PixelToGridX((int)mPosX, (int)mPosY);

		if (mLastTileX == -1)
		{
			mLastTileX = aCurrentTileX;
		}

		if (aCurrentTileX != mLastTileX)
		{
			mLastTileX = aCurrentTileX;
			TryZigZagRowChange();
		}
...
void Zombie::TryZigZagRowChange()
{
    // 1. تصفية أنواع الزومبي المسموح لها بالحركة
    bool aCanZigZag = (mZombieType == ZombieType::ZOMBIE_NORMAL ||
                       mZombieType == ZombieType::ZOMBIE_PEA_HEAD ||
                       mZombieType == ZombieType::ZOMBIE_WALLNUT_HEAD ||
    ...
    // 2. التحقق من المسار العلوي والسفلي (نفس منطق Garlic)
    bool aCanGoUp = true;
    bool aCanGoDown = true;
    bool aIsPool = (mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL);
    ...
    // فحص الصف العلوي
    if (!mBoard->RowCanHaveZombies(mRow - 1) || 
       (mBoard->mPlantRow[mRow - 1] == PlantRowType::PLANTROW_POOL && !aIsPool) ||
       (mBoard->mPlantRow[mRow - 1] != PlantRowType::PLANTROW_POOL && aIsPool))
    {
        aCanGoUp = false;
    }
    ...
    // 3. تغيير الصف عشوائياً بناءً على الخيارات المتاحة
    if (aCanGoDown && !aCanGoUp)
    {
        SetRow(mRow + 1);
    }
...
{
	return mIceTrapCounter > 0 || mButteredCounter > 0 || mIsPulledByPotato;
}
