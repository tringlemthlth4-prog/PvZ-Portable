...
void Plant::PullHalfZombiesToPotato()
{
	...
	float aPotatoPosY = static_cast<float>(mBoard->GridToPixelY(mPlantCol, mRow));
...
void Plant::UpdateChomper()
{
	...
	else if (mState == PlantState::STATE_CHOMPER_BITING)
	{
		if (mStateCountdown == 0)
		{
			...
			{
				// إذا كان الهدف عملاقاً: لا يتم أكله بل يوجه له ضربة عض عادية بضرر 40
				mApp->PlayFoley(FoleyType::FOLEY_SPLAT);
			...
			{
				// --- تجميع أقرب 3 زومبي في الصف بشرط ألا يكونوا عمالقة ---
				...
				for (Zombie* aZombie : mBoard->mZombies)
				{
					// التأكد من وجود الزومبي، وأنه في نفس الصف، وغير ميت، وليس عملاقاً
					if (aZombie && 
						!aZombie->mDead && 
						aZombie->mRow == mRow && 
					...
					{
						// التحقق من أن الزومبي ضمن نطاق هجوم التشومبير في الصف
						if (mBoard->ZombiesInAggroRange(aZombie, this, PlantWeapon::WEAPON_PRIMARY))
						{
							...
							{
								break; // التوقف فور الوصول لـ 3 زومبي
							}
						}
					}
				}

				// أكل كافة الزومبي المجمعين (من 1 إلى 3 زومبي)
