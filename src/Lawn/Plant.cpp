...
constinit const PlantDefinition gPlantDefs[SeedType::NUM_SEED_TYPES] = {
	{ .mSeedType = SeedType::SEED_PEASHOOTER,        .mPlantImage = nullptr, .mReanimationType = ReanimationType::REANIM_PEASHOOTER,    .mPacketIndex = 0,  .mSeedCost = 0, .mRefreshTime = 750,    .mSubClass = PlantSubClass::SUBCLASS_SHOOTER, .mLaunchRate = 150,  .mPlantName = "PEASHOOTER" },
	...
	{ .mSeedType = SeedType::SEED_SNOWPEA,           .mPlantImage = nullptr, .mReanimationType = ReanimationType::REANIM_SNOWPEA,       .mPacketIndex = 4,  .mSeedCost = 175, .mRefreshTime = 750,    .mSubClass = PlantSubClass::SUBCLASS_SHOOTER, .mLaunchRate = 500,  .mPlantName = "SNOW_PEA" },
...
void Plant::PlantInitialize(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
	...
	if (aPlantDef.mReanimationType != ReanimationType::REANIM_NONE)
	{
		...
		{
            mState = Sexy::Rand(2) ? PlantState::STATE_BOWLING_UP : PlantState::STATE_BOWLING_DOWN;
...
void Plant::PullHalfZombiesToPotato()
{
	if (!mBoard) 
		return;

	// 1. تجميع الزومبي الأحياء باستخدام سعة آمنة وثابتة
	const int MAX_PULL_CAPACITY = 500;
	Zombie* aLivingZombies[MAX_PULL_CAPACITY];
	int aLivingCount = 0;

	for (Zombie* aZombie : mBoard->mZombies)
	{
		if (aZombie && !aZombie->mDead && aLivingCount < MAX_PULL_CAPACITY)
		{
			aLivingZombies[aLivingCount] = aZombie;
			aLivingCount++;
		}
	}
	...
	// 2. حساب نصف العدد (زومبي واحد على الأقل إن وجد)
	int aTargetCount = aLivingCount / 2;
	if (aTargetCount == 0 && aLivingCount > 0)
	{
		aTargetCount = 1;
	}

	// 3. خلط الترتيب عشوائياً باستخدام Sexy::Rand المتوافقة مع المحرك
	for (int i = aLivingCount - 1; i > 0; i--)
	{
		int j = Sexy::Rand(i + 1);
		Zombie* aTemp = aLivingZombies[i];
		aLivingZombies[i] = aLivingZombies[j];
		aLivingZombies[j] = aTemp;
	}

	// 4. تعيين موقع هدف السحب وتفعيل شل الحركة للزومبي المختارين
	float aPotatoPosX = static_cast<float>(mX);
	float aPotatoPosY = static_cast<float>(mBoard->GridToPixelY(mPlantCol, mRow)) + 40.0f;

	for (int i = 0; i < aTargetCount; i++)
	{
		Zombie* aZombie = aLivingZombies[i];

		aZombie->mPullTargetX = aPotatoPosX;
		aZombie->mPullTargetY = aPotatoPosY;
		aZombie->mPullTargetRow = mRow;
		aZombie->mIsPulledByPotato = true;
	}
}
...
void Plant::UpdateChomper()
{
	...
	else if (mState == PlantState::STATE_CHOMPER_BITING)
	{
		if (mStateCountdown == 0)
		{
			...
			else if (!aTarget->IsImmobilizied())
			{
				if (aTarget->IsBouncingPogo() ||
					aTarget->mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_IN_VAULT || 
					aTarget->mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT)
				{
					doMiss = true;
				}
			}
			...
			else if (aTarget->mZombieType == ZombieType::ZOMBIE_GARGANTUAR || 
					 aTarget->mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR ||
					 aTarget->mZombieType == ZombieType::ZOMBIE_BOSS)
			{
				// ضربة عض عادية للعمالقة بضرر 40
				mApp->PlayFoley(FoleyType::FOLEY_SPLAT);
				aTarget->TakeDamage(40, 0U);
				mState = PlantState::STATE_CHOMPER_BITING_MISSED;
			}
			else
			{
				// --- تجميع أقرب 3 زومبي في الصف الكامل أمام التشومبير ---
				const int MAX_TARGETS = 3;
				Zombie* aTargetsToEaten[MAX_TARGETS];
				int aEatenCount = 0;
				...
				for (Zombie* aZombie : mBoard->mZombies)
				{
					...
					{
						// حساب المسافة الأفقية: التقاط أي زومبي متواجد أمامه على طول الخط
						float aDiffX = aZombie->mX - mX;
						if (aDiffX >= -40.0f) // يشمل الصف بأكمله إلى نهاية الخريطة
						{
							aTargetsToEaten[aEatenCount] = aZombie;
							aEatenCount++;

							if (aEatenCount >= MAX_TARGETS)
							{
								break; // الاكتفاء بأول 3 زومبي في الصف
							}
						}
					}
				}

				// أمان: إذا لم يجد بالحساب، أكل الهدف الأساسي المكتشف
				if (aEatenCount == 0 && aTarget)
				{
					aTargetsToEaten[0] = aTarget;
					aEatenCount = 1;
				}

				// أكل الزومبي المحددين
				for (int i = 0; i < aEatenCount; i++)
				{
					aTargetsToEaten[i]->DieWithLoot();
				}
...
void Plant::UpdateBowling()
{
	...
	if (aDistToGrid < -5 || aDistToGrid > 5)
		return;
	...
	if (aZombie)
	{
		...
		{
			// 1. تشغيل صوت الدوم شروم الاصلي
			mApp->PlaySample(SOUND_DOOMSHROOM);

			// 2. تطبيق الضرر الممتد ونفس نصف القطر (250) مع تأثير الدوم شروم
...
void Plant::DoSpecial()
{
	...
	case SeedType::SEED_DOOMSHROOM:
	{
		...
		// تنفيذ الضرر والتحقق من الانجاز للكرز
		int aZombiesKilled = mBoard->KillAllZombiesInRadius(mRow, aPosX, aPosY, 250, 3, true, aDamageRangeFlags);
		if (mSeedType == SeedType::SEED_CHERRYBOMB && aZombiesKilled >= 10)
		{
		...
		// إضافة الحفرة فقط إذا لم يكن النبات كرزاً (أي للدوم شورم فقط)
...
Rect Plant::GetPlantAttackRect(PlantWeapon thePlantWeapon)
{
	...
	case SeedType::SEED_PEASHOOTER:      aRect = Rect(mX + 60,       mY,             400,                mHeight);               break;
	case SeedType::SEED_PUFFSHROOM:
