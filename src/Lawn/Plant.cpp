...
void Plant::PullHalfZombiesToPotato()
{
	...
	// 1. تجميع الزومبي الأحياء باستخدام مصفوفة ثابتة
	Zombie* aLivingZombies[MAX_ZOMBIES];
	int aLivingCount = 0;

	for (Zombie* aZombie : mBoard->mZombies)
	{
		if (aZombie && !aZombie->mDead && aZombie->mZombiePhase != ZombiePhase::ZOMBIE_PHASE_DEAD)
		{
	...
	// 3. خلط الترتيب باستخدام std::random_shuffle الموجودة في <algorithm>
	std::random_shuffle(aLivingZombies, aLivingZombies + aLivingCount);

	// 4. نقل الزومبي وشل حركتهم
	...
	for (int i = 0; i < aTargetCount; i++)
	{
		...
		aZombie->mRow = mRow;
		aZombie->mPosY = static_cast<float>(mBoard->GridToPixelY(mPlantCol, mRow));
		aZombie->mPosX = aPotatoPosX;
